...
class AddCategoryDestinationParams(Params):
...
class AddCategoryDestinationOutput(PermissiveActionOutput):
    id: str | None = None
    val: float | None = None
    type: str | None = None
...
def add_category_destination(
    params: AddCategoryDestinationParams, soar: SOARClient, asset: Asset
) -> AddCategoryDestinationOutput:
    values = [
    ...
    try:
        with get_client(asset) as client:
            ...
            result = AddCategoryDestinationOutput(**raw_updated)
        ...
        logger.exception("Add category destination failed")
        message = f"Add category destination failed: {exc}"
        soar.set_message(message)
        raise ActionFailure(message) from exc
    soar.set_message("Category destinations updated")
    return result

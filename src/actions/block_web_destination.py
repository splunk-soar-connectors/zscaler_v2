...
class BlockWebDestinationParams(Params):
...
class BlockWebDestinationOutput(PermissiveActionOutput):
    pass


class BlockWebDestinationSummary(PermissiveActionOutput):
    ignored: list[str] = OutputField(example_values=["test example.com"])
    updated: list[str] = OutputField(example_values=["test 192.0.2.10"])


def _summary(updated: list[str], ignored: list[str]) -> BlockWebDestinationSummary:
    return BlockWebDestinationSummary(updated=updated, ignored=ignored)


def block_web_destination(
    params: BlockWebDestinationParams, soar: SOARClient, asset: Asset
) -> list[BlockWebDestinationOutput]:
    ...
    try:
        with get_client(asset) as client:
            ...
            if not updated:
                ...
                soar.set_message("Blocklist contains all of these destinations")
                return []

            changed, _changed_response, change_error = (
                client.zia.security_policy_settings.add_urls_to_blacklist(updated)
            )
        ...
        logger.exception("Block web destination failed")
        message = f"Block web destination failed: {exc}"
        soar.set_message(message)
        raise ActionFailure(message) from exc

...
class AllowUrlOutput(PermissiveActionOutput):
...
def allow_url(
    params: AllowUrlParams, soar: SOARClient, asset: Asset
) -> list[AllowUrlOutput]:
    endpoints = [
    ...
    try:
        ...
        with get_client(asset) as client:
            if params.url_category:
                ...
                rows.append(AllowUrlOutput(**raw_result))
            else:
        ...
        logger.exception("Allow URL failed")
        message = f"Allow URL failed: {exc}"
        soar.set_message(message)

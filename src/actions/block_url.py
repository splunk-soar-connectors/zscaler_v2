...
class BlockUrlOutput(PermissiveActionOutput):
...
class BlockUrlSummary(PermissiveActionOutput):
...
def _summary(updated: list[str], ignored: list[str]) -> BlockUrlSummary:
    return BlockUrlSummary(**cast(Any, {"updated": updated, "ignored": ignored}))


def block_url(
    params: BlockUrlParams, soar: SOARClient, asset: Asset
) -> list[BlockUrlOutput]:
    endpoints = [
    ...
    try:
        ...
        with get_client(asset) as client:
            if params.url_category:
                ...
                rows.append(BlockUrlOutput(**raw_result))
            else:
                ...
                updated = list(set(endpoints) - set(settings.blacklist_urls))
                ignored = list(set(endpoints) - set(updated))
                if not updated:
        ...
        logger.exception("Block URL failed")
        message = f"Block URL failed: {exc}"
        soar.set_message(message)

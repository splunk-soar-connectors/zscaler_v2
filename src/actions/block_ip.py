...
class BlockIpOutput(PermissiveActionOutput):
...
class BlockIpSummary(PermissiveActionOutput):
    ignored: str | None = OutputField(example_values=["test 8.8.8.8"])
    updated: str | None = OutputField(example_values=["test 208.67.222.222"])


def _summary(updated: list[str], ignored: list[str]) -> BlockIpSummary:
    return BlockIpSummary(**cast(Any, {"updated": updated, "ignored": ignored}))


def block_ip(
    params: BlockIpParams, soar: SOARClient, asset: Asset
) -> list[BlockIpOutput]:
    endpoints = [
    ...
    try:
        ...
        with get_client(asset) as client:
            if params.url_category:
                ...
                rows.append(BlockIpOutput(**raw_result))
            else:
        ...
        logger.exception("Block IP failed")
        message = f"Block IP failed: {exc}"
        soar.set_message(message)

...
def test_add_category_destination_requires_a_destination(
    connector_app: App,
...
def test_add_category_destination_replaces_old_actions() -> None:
    actions = {
        action.identifier: action
        for action in create_zscaler_soar_connector_app().actions_manager.get_actions_meta_list()
    }
    assert "add_category_ip" not in actions
    assert "add_category_url" not in actions
    action = actions["add_category_destination"]

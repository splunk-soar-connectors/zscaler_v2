...
_TEST_URL = "papp-38277-remove-category.example"
_TEST_PARENT_URL = "papp-38277-remove-parent-category.example"


def test_remove_category_url_live_removes_activates_and_preserves_contract(
    connector_app: App,
    ...
    try:
        ...
        with get_client(asset) as client:
            ...
            assert _TEST_URL not in changed["urls"]
            assert _TEST_PARENT_URL not in changed["dbCategorizedUrls"]
    finally:
        with get_client(asset) as client:
            ...
            if _TEST_URL in current.get("urls", []) or _TEST_PARENT_URL in current.get(
                "dbCategorizedUrls", []
            ):

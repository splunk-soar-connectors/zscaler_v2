...
def test_add_category_ip_live_updates_activates_and_returns_oneapi_lists(
    ...
    with get_client(asset) as client:
        ...
        assert _TEST_IP not in category.get("urls", [])
        assert _TEST_PARENT_IP not in category.get("dbCategorizedUrls", [])
    ...
    try:
        ...
        with get_client(asset) as client:
            ...
            assert _TEST_IP in changed["urls"]
            assert _TEST_PARENT_IP in changed["dbCategorizedUrls"]
    finally:
        with get_client(asset) as client:
